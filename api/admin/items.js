...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'list') {
      const { category_type, gold_type, karat, q } = req.query || {}
      let query = admin
        .from('items')
        .select('*')
    ...
    if (action === 'create') {
      const { name, purchase_price, sell_price, total_quantity, image_url, status = 'active', discount_type = 'none', discount_value = 0, category_type = null, gold_type = null, karat = null, description = null, restock_threshold = null } = req.body || {}
      ...
      const payload = {
        ...
        karat,
        description,
        restock_threshold: restock_threshold != null ? Number(restock_threshold) : null
      }
  ...
  if (restock_threshold !== undefined) patch.restock_threshold = (restock_threshold === null || restock_threshold === '') ? null : Number(restock_threshold)
